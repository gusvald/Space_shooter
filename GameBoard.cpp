...
void GameBoard::Logic(sf::RenderWindow &win, Spaceship &player) {
    ...
    info(player);
    lasers(player);
    playerPos(player, win);
    asteroids(clk, clk2, win);
    colisions(win, player);
...
void GameBoard::movement(Spaceship &player) {
...
void GameBoard::info(Spaceship &player) {
...
void GameBoard::lasers(Spaceship &player) {
    for (unsigned int i = 0; i < player.lasers.size(); i++) {
...
void GameBoard::playerPos(Spaceship &player, sf::RenderWindow &win) {
...
void GameBoard::asteroids(sf::Clock &clk, sf::Clock &clk2, sf::RenderWindow &win) {
...
void GameBoard::colisions(sf::RenderWindow &win, Spaceship &player) {
    for (unsigned int i = 0; i < space.objects.size(); i++) {
